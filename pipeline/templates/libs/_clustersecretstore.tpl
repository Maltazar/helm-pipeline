{{- define "libs.clustersecretstore.tpl" -}}
{{- $all := index . 0 -}}
{{- $app := index . 1 -}}
{{- $v := index . 2 -}}

{{- range $name, $val := $v.clustersecretstores }}
  {{- if kindIs "map" $val }}
...
{{- define "libs.clustersecretstore" -}}
{{- $all := index . 0 -}}
{{- $app := index . 1 -}}
{{- include "helper.no.merge" (list $all $app "clustersecretstore" ) -}}
{{- end -}}

{{- define "libs.clustersecretstore.otherContainters" -}}
{{- include "helper.merge.otherContainters" (list . "clustersecretstore" ) -}}
{{- end -}} 