{{- define "libs.statefulset.tpl" -}}
{{- $all := index . 0 -}}
{{- $app := index . 1 -}}
{{- $v := index . 2 -}}

{{- range $name, $val := $v.statefulsets }}
  {{- if kindIs "map" $val }}
...
spec:
  ...
  {{- if $val.podManagementPolicy }}
  podManagementPolicy: {{ $val.podManagementPolicy }}
  {{- end }}
...
{{ toYaml $val.volumes | indent 8 }}
  ...
  {{- range $pvc := $val.volumeClaimTemplates }}
    - metadata:
        name: {{ $pvc.name }}
      spec:
        accessModes:
...
{{- define "libs.statefulset" -}}
{{- $all := index . 0 -}}
{{- $app := index . 1 -}}
{{- include "helper.no.merge" (list $all $app "statefulset" ) -}}
{{- end -}}

{{- define "libs.statefulset.otherContainters" -}}
{{- include "helper.merge.otherContainters" (list . "statefulset" ) -}}
{{- end -}} 